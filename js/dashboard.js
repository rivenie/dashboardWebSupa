...
function col(clave) {
  ...
  return keys.find(
    (k) => k.trim().toLowerCase() === clave.trim().toLowerCase(),
  );
}
function norm(v) {
...
function ordenarMeses(labels) {
  ...
  return labels.sort(
    (a, b) => orden.indexOf(a.toUpperCase()) - orden.indexOf(b.toUpperCase()),
  );
}
  ...
  charts[id] = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          data,
          backgroundColor: PALETTE_DONUT.slice(0, labels.length),
          borderColor: "#FFFFFF",
          borderWidth: 3,
        },
      ],
...
function llenar(id, columna) {
  ...
  if (!c) return;
  const valores = [
    ...new Set(dataGlobal.map((f) => norm(f[c])).filter((v) => v !== "")),
  ];
...
function crearGraficos() {
  ...
  renderLine(
    "chartFactMes",
    mesesOrdenados,
    mesesOrdenados.map((m) => porMesUSD[m]),
    COLORS.primary,
  );
  ...
  renderLine(
    "chartDespMes",
    mesesOrdenados,
    mesesOrdenados.map((m) => porMesDesp[m]),
    COLORS.primaryDark,
  );
  ...
  renderLine(
    "chartVolMes",
    mesesOrdenados,
    mesesOrdenados.map((m) => porMesVol[m]),
    COLORS.green,
  );
  ...
  renderLine(
    "chartDespSem",
    semanasOrdenadas,
    semanasOrdenadas.map((s) => porSemana[s]),
    COLORS.orange,
  );
  ...
  renderDoughnut(
    "chartTransporte",
    Object.keys(porTransp),
    Object.values(porTransp),
  );
  ...
  const paisesArr = Object.keys(porPaisUSD)
    .sort((a, b) => porPaisUSD[b] - porPaisUSD[a])
    .slice(0, 10);
  renderCombo(
    "chartPais",
    paisesArr,
    paisesArr.map((p) => porPaisUSD[p]),
    paisesArr.map((p) => porPaisDesp[p]),
    COLORS.primary,
    COLORS.orange,
  );
  ...
  const incArr = Object.keys(porIncUSD).sort(
    (a, b) => porIncUSD[b] - porIncUSD[a],
  );
  renderCombo(
    "chartIncoterm",
    incArr,
    incArr.map((i) => porIncUSD[i]),
    incArr.map((i) => porIncDesp[i]),
    COLORS.primaryDark,
    COLORS.green,
  );
}
...
function crearTablas() {
  ...
  const clientesArr = Object.entries(porCliente).sort(
    (a, b) => b[1].usd - a[1].usd,
  );
  ...
  const bottom5 = clientesArr
    .filter((c) => c[1].usd > 0)
    .slice(-5)
    .reverse();

  document.getElementById("tablaTopClientes").innerHTML =
    generarTablaHTML(top5);
  document.getElementById("tablaBottomClientes").innerHTML =
    generarTablaHTML(bottom5);
}

function generarTablaHTML(arr) {
  let html =
    "<table><thead><tr><th>Cliente</th><th>Venta $</th><th>Venta S/</th><th>Desp.</th></tr></thead><tbody>";
